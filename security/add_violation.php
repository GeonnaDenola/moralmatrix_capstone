...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['evidence']) && is_array($_FILES['evidence']['name'])) {
        $uploadDir = __DIR__ . "/uploads/"; // security/uploads/
        if (!is_dir($uploadDir)) {
        ...
        for ($i = 0; $i < $count && count($evidence_paths) < $maxFiles; $i++) {
            ...
            if (move_uploaded_file($_FILES['evidence']['tmp_name'][$i], $uploadDir . $filename)) {
                // store path under security/uploads/
                $relPath = 'security/uploads/' . $filename;
    ...
    // --- NOTIFY CCDU ONLY (Security report, pending) ---
    $actorId = $_SESSION['actor_id'] ?? 'unknown';

    Notify::create($conn, [
        'type'         => 'warning',
        'target_role'  => 'ccdu',
        'title'        => 'Violation reported by Security',
    ...
    header('Location: ' . asset('security/view_student.php?student_id=' . urlencode($student_id) . '&saved=1'));
    ob_end_flush();
    exit;
}
/* ========= END INSERT HANDLER ========= */
